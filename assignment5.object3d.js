...
class Object3D
{
    ...
    createVAO( gl, shader )
    {
        ...
        if (this instanceof Box) {
            ...
            gl.vertexAttribPointer(shader.getAttributeLocation( "a_position" ), this.num_components, gl.FLOAT, false, 0, 0 )
        }
        else {
            let posArray = []
            let normalArray = []
            for (let i = 0; i < this.vertices.length; i += 2) {
                posArray.push(this.vertices[i])
                normalArray.push(this.vertices[i+1])
            }
            ...
            if (shader.getAttributeLocation( "a_normal" ) >= 0) {
                ...
                gl.vertexAttribPointer(shader.getAttributeLocation( "a_normal" ), this.num_components, gl.FLOAT, false, 0, 0);
            }
        }
