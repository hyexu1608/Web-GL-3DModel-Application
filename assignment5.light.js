...
class DirectionalLight extends Light {
    ...
    update( ) {
        ...
        let quaternion = quat4.create()
        mat4.getRotation(quaternion, this.model_matrix)
        let rotateAxisAngle = vec3.create()
        quat4.getAxisAngle(rotateAxisAngle, quaternion)
        
        let lightDirection = [0, -1, 0, 1]
        let rotateX = mat4.create()
        let rotateY = mat4.create()
        let rotateZ = mat4.create()
        mat4.fromXRotation(rotateX, rotateAxisAngle[0])
        mat4.fromYRotation(rotateY, rotateAxisAngle[1])
        mat4.fromZRotation(rotateZ, rotateAxisAngle[2])
        
        vec4.transformMat4(lightDirection, lightDirection, rotateX)
        vec4.transformMat4(lightDirection, lightDirection, rotateY)
        vec4.transformMat4(lightDirection, lightDirection, rotateZ)

        lightDirection = [lightDirection[0], lightDirection[1], lightDirection[2]]
        this.target_shader.setUniform3f(`u_lights_directional[${this.id}].direction`, lightDirection)
        this.target_shader.unuse()
...
class PointLight extends Light {
    ...
    update( ) {
        ...
        let lightPos = [this.model_matrix[12], this.model_matrix[13], this.model_matrix[14]]
        ...
        this.target_shader.setUniform3f(`u_lights_point[${this.id}].position`, lightPos)
        this.target_shader.unuse()
    }
}
