...
class OBJLoader {
    ...
    parseVec3(vec_string) {
        let lineArray = vec_string.split(' ');
    ...
    parseFace(face_string, entry_index)
    {
        ...
        if (lineArray.length == 4){ //triangle face
            return [parseInt(lineArray[1].split('/')[entry_index])-1, 
                              parseInt(lineArray[2].split('/')[entry_index])-1, 
                              parseInt(lineArray[3].split('/')[entry_index])-1];
        }
        else if (lineArray.length == 5){ //quad face
            let triangulated = this.triangulateFace([parseInt(lineArray[1].split('/')[entry_index])-1, 
                                                          parseInt(lineArray[2].split('/')[entry_index])-1, 
                                                          parseInt(lineArray[3].split('/')[entry_index])-1,
                                                          parseInt(lineArray[4].split('/')[entry_index])-1]);
            return triangulated;
        }
